...
import { loadApiSettings, saveApiSettings, ApiSettings } from '../utils/apiSettings';
...
export default function AIOptimizeModal({
  ...
  const handleOptimize = useCallback(async () => {
    ...
    try {
      const prompt = settings.defaultPrompt.trim();
      if (!prompt) {
        setError('请输入优化提示词。');
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="mb-4 border border-gray-200 dark:border-gray-700 rounded-lg">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">
                  ({settings.provider === 'doubao' ? '豆包 Seed' : '自定义API'})
                </span>
              </div>
              <svg className={`w-4 h-4 text-gray-400 transition-transform ${showConfig ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showConfig && (
              <div className="px-4 pb-4 pt-1 border-t border-gray-100 dark:border-gray-700 space-y-3">
                {/* 供应商选择 */}
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">供应商</label>
                  <select
                    value={settings.provider}
                    onChange={e => updateSettings({ provider: e.target.value as ApiSettings['provider'] })}
                  ...
                  >
                    <option value="doubao">豆包 Seed 1.5</option>
                    <option value="custom">自定义 API</option>
                  </select>
                </div>

                {/* API Key 输入（豆包和自定义都需要） */}
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                    API Key {settings.provider === 'doubao' ? '(豆包 Ark API Key)' : ''}
                  </label>
                  <input
                    type="password"
                    value={settings.apiKey}
                    onChange={e => updateSettings({ apiKey: e.target.value })}
                    placeholder={settings.provider === 'doubao' ? '从火山引擎 Ark 控制台获取' : 'sk-...'}
                    className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
                {/* API 端点 URL（自定义时显示） */}
                {settings.provider === 'custom' && (
                  <div>
                    <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">API 端点 URL</label>
                    <input
                      type="text"
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">设置自动保存至本地浏览器</p>
              </div>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm resize-none h-24 focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:opacity-50"
            />
