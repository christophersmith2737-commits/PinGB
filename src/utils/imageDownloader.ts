...
export async function downloadImage({
  ...
  const processDownload = () => {
    ...
    // 从下载选项中获取设置
    const { showGrid, coarseGridInterval, fineGridInterval, showCoordinates, gridLineColor, includeStats, showCellNumbers = true } = options;
    ...
    // 主标题位置
    const titleStartX = titleBarHeight * 0.3;
    const mainTitleY = titleBarHeight * 0.3;
    
    ctx.fillText('拼好豆 PinGB', titleStartX, mainTitleY);
    ...
    ctx.fillText('AI 拼豆生成器', titleStartX, subTitleY);
    ...
    if (showCoordinates) {
      ...
      ctx.textBaseline = 'middle';
      for (let i = 0; i < N; i++) {
        if ((i + 1) % coarseGridInterval === 0 || i === 0 || i === N - 1) { // 在间隔处、起始处和结束处标注
          // 将数字放在轴线之上，考虑额外边距
      ...
      ctx.textBaseline = 'middle';
      for (let j = 0; j < M; j++) {
        if ((j + 1) % coarseGridInterval === 0 || j === 0 || j === M - 1) { // 在间隔处、起始处和结束处标注
          // 将数字放在轴线之左
    ...
    if (showGrid) {
      ...
      ctx.lineWidth = 0.5;
      const fineInterval = fineGridInterval || Math.floor(coarseGridInterval / 2);

      // 垂直细虚线
      for (let i = fineInterval; i < N; i += fineInterval) {
        // 跳过与粗线重合的位置
        if (i % coarseGridInterval === 0) continue;
      ...
      // 水平细虚线
      for (let j = fineInterval; j < M; j += fineInterval) {
        if (j % coarseGridInterval === 0) continue;
      ...
      // 绘制垂直粗实线 - 在单元格之间
      for (let i = coarseGridInterval; i < N; i += coarseGridInterval) {
        const lineX = extraLeftMargin + i * downloadCellSize + axisLabelSize;
        ctx.beginPath();
        ctx.moveTo(lineX, titleBarHeight + extraTopMargin + axisLabelSize);
        ctx.lineTo(lineX, titleBarHeight + extraTopMargin + axisLabelSize + M * downloadCellSize);
      ...
      // 绘制水平粗实线 - 在单元格之间
      for (let j = coarseGridInterval; j < M; j += coarseGridInterval) {
        const lineY = titleBarHeight + extraTopMargin + j * downloadCellSize + axisLabelSize;
        ctx.beginPath();
        ctx.moveTo(extraLeftMargin + axisLabelSize, lineY);
        ctx.lineTo(extraLeftMargin + axisLabelSize + N * downloadCellSize, lineY);
    ...
    // 绘制统计信息
    if (includeStats && colorCounts) {
      const colorKeys = Object.keys(colorCounts).sort((a, b) => colorCounts[b].count - colorCounts[a].count);
