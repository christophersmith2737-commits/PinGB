...
import { addOuterBorder } from '../utils/borderUtils';
import RecolorPopover from '../components/RecolorPopover';
...
import { GridCell, addCellsToMask, clearMask } from '../utils/selectionUtils';
import { createRng } from '../utils/particleUtils';
...
export default function Home() {
  ...
  // 替换颜色：记录哪个被排除的颜色正在被编辑替换
  const [replacingHexKey, setReplacingHexKey] = useState<string | null>(null);
  const [replaceInputValue, setReplaceInputValue] = useState<string>('');
  // 保存排除前的格子坐标（hex → 坐标列表），用于手动替换时精确还原
  const preExclusionCellsRef = useRef<Map<string, { r: number; c: number }[]>>(new Map());
  ...
  // 新增：智能噪点识别状态
  const [suspectedNoiseKeys, setSuspectedNoiseKeys] = useState<Set<string>>(new Set());
  const [highlightNoiseHexes, setHighlightNoiseHexes] = useState<Set<string> | null>(null); // null=不高亮, Set=高亮这些hex的像素
  ...
  const handleAddBorder = useCallback(() => {
    ...
    const borderHex = convertColorKeyToHex('H07', selectedColorSystem);
    const borderColor: MappedPixel = { key: borderHex, color: borderHex, isExternal: false };
    const newData = addOuterBorder(mappedPixelData, gridDimensions, borderColor);
    setMappedPixelData(newData);
    const { colorCounts: newCounts, totalCount } = recalculateColorStats(newData);
    setColorCounts(newCounts);
    setTotalBeadCount(totalCount);
  }, [mappedPixelData, gridDimensions, selectedColorSystem]);

  // 改色模式 state
  const [isRecolorMode, setIsRecolorMode] = useState(false);
  const [recolorSelected, setRecolorSelected] = useState<{row: number; col: number} | null>(null);
  const [recolorSnapshot, setRecolorSnapshot] = useState<MappedPixel[][] | null>(null);
  const [recolorUndoStack, setRecolorUndoStack] = useState<MappedPixel[][][]>([]);
  const [recolorPopoverStep, setRecolorPopoverStep] = useState<'menu' | 'colorOptions' | 'replaceOptions'>('menu');
  const [recolorPendingHex, setRecolorPendingHex] = useState<string | null>(null);

  // 改色模式 — 进入/退出
  const handleRecolorModeToggle = useCallback(() => {
    if (!isRecolorMode) {
      if (mappedPixelData) {
        setRecolorSnapshot(mappedPixelData.map(row => row.map(cell => ({ ...cell }))));
        setRecolorUndoStack([]);
      }
      setRecolorSelected(null);
      setRecolorPopoverStep('menu');
      setRecolorPendingHex(null);
      setIsRecolorMode(true);
      setIsManualColoringMode(false);
      setIsBrushMode(false);
    } else {
      const hasChanges = recolorUndoStack.length > 0;
      if (hasChanges) {
        const confirmed = window.confirm('是否保存改色更改？\n"确定"保存，"取消"恢复到改色前状态。');
        if (!confirmed) {
          if (recolorSnapshot) {
            setMappedPixelData(recolorSnapshot);
            const stats = recalculateColorStats(recolorSnapshot);
            setColorCounts(stats.colorCounts);
            setTotalBeadCount(stats.totalCount);
          }
        }
      }
      setIsRecolorMode(false);
      setRecolorSelected(null);
      setRecolorSnapshot(null);
      setRecolorUndoStack([]);
    }
  }, [isRecolorMode, mappedPixelData, recolorSnapshot, recolorUndoStack]);

  const handleRecolorPixelSelect = useCallback((row: number, col: number) => {
    if (!isRecolorMode || !mappedPixelData || !gridDimensions) return;
    const cell = mappedPixelData[row]?.[col];
    if (!cell || cell.isExternal || cell.key === TRANSPARENT_KEY) return;
    setRecolorSelected({ row, col });
    setRecolorPopoverStep('menu');
    setRecolorPendingHex(null);
  }, [isRecolorMode, mappedPixelData, gridDimensions]);

  const handleRecolorSelectColor = useCallback((hex: string) => {
    if (hex === '__SHOW_OPTIONS__') {
      setRecolorPopoverStep('colorOptions');
      return;
    }
    setRecolorPendingHex(hex);
    setRecolorPopoverStep('replaceOptions');
  }, []);

  const handleRecolorConfirmReplace = useCallback((scope: 'all' | 'single') => {
    if (!mappedPixelData || !recolorSelected || !recolorPendingHex || !gridDimensions) return;
    const { row, col } = recolorSelected;
    const oldData = mappedPixelData.map(r => r.map(c => ({ ...c })));
    setRecolorUndoStack(prev => [...prev, oldData]);

    const newData = mappedPixelData.map(r => r.map(c => ({ ...c })));
    const targetColor = mappedPixelData[row][col].color.toUpperCase();

    if (scope === 'all') {
      for (let j = 0; j < gridDimensions.M; j++) {
        for (let i = 0; i < gridDimensions.N; i++) {
          if (newData[j][i].color.toUpperCase() === targetColor && !newData[j][i].isExternal) {
            newData[j][i] = { key: recolorPendingHex, color: recolorPendingHex, isExternal: false };
          }
        }
      }
    } else {
      newData[row][col] = { key: recolorPendingHex, color: recolorPendingHex, isExternal: false };
    }
    ...
    setRecolorPopoverStep('menu');
    setRecolorPendingHex(null);
  }, [mappedPixelData, recolorSelected, recolorPendingHex, gridDimensions]);

  // Ctrl+Z 撤销（改色模式）
  useEffect(() => {
    if (!isRecolorMode) return;
    const handler = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.key === 'z' && !e.shiftKey && !e.metaKey) {
        e.preventDefault();
        setRecolorUndoStack(prev => {
          if (prev.length === 0) return prev;
          const last = prev[prev.length - 1];
          setMappedPixelData(last);
          const stats = recalculateColorStats(last);
          setColorCounts(stats.colorCounts);
          setTotalBeadCount(stats.totalCount);
  ...
  // 新增：智能抠图弹窗状态
  const [isBgRemoveOpen, setIsBgRemoveOpen] = useState<boolean>(false);
  const [originalFileName, setOriginalFileName] = useState<string>('');

  // --- 笔刷选择 & 消散状态 ---
  const [selectionMask, setSelectionMask] = useState<Set<string>>(new Set());
  const [brushRadius, setBrushRadius] = useState<number>(3);
  const [isBrushMode, setIsBrushMode] = useState<boolean>(false);
  const [isDissolvePanelOpen, setIsDissolvePanelOpen] = useState<boolean>(false);
  const [dissolvePanelActive, setDissolvePanelActive] = useState<boolean>(false);
  ...
  const handleBrushToggle = () => {
    ...
    setIsBrushMode(next);
    if (next) {
      setIsDissolvePanelOpen(false);
      setDissolvePanelActive(false);
    }
  };

  // 笔刷选择：拖拽更新选区（过滤背景/外部格子）
  const handleBrushStroke = (cells: GridCell[], isAdditive: boolean) => {
    const validCells = mappedPixelData
    ...
    setSelectionMask(prev => isAdditive ? addCellsToMask(prev, validCells) : addCellsToMask(clearMask(), validCells));
  };
  ...
  // --- 撤销系统 ---
  interface UndoEntry {
    mappedPixelData: typeof mappedPixelData;
    colorCounts: typeof colorCounts;
  ...
  const pushUndo = () => {
    ...
    undoStackRef.current.push({
      ...
      colorCounts: colorCounts ? { ...colorCounts } : null,
      totalBeadCount,
      initialGridColorKeys: new Set(initialGridColorKeys),
    });
    // 最多保留 30 步
  ...
  const handleUndo = () => {
    ...
    if (!entry) return;
    setUndoCount(undoStackRef.current.length);
    setMappedPixelData(entry.mappedPixelData);
    setColorCounts(entry.colorCounts);
    setTotalBeadCount(entry.totalBeadCount);
    setInitialGridColorKeys(entry.initialGridColorKeys);
    setSelectionMask(new Set());
  };

  // 消散：存储参数 → 触发 useEffect（等 canvas 渲染到 DOM 后再启动动画）
  // 消散：粒子沿角度方向飞散，留在画布上
  const handleDissolveApply = (params: { angle: number; intensity: number; seed: number }) => {
    if (!mappedPixelData || !gridDimensions || selectionMask.size === 0) return;
    pushUndo();

    const { N, M } = gridDimensions;
    const rng = createRng(params.seed);
    const angleRad = (params.angle * Math.PI) / 180;
    const dirCol = Math.cos(angleRad);
    const dirRow = -Math.sin(angleRad); // canvas y flipped
    ...
    // 最大偏移距离（格子数）
    const maxDist = Math.max(1, Math.round((params.intensity / 100) * 15));

    const newData = mappedPixelData.map((row) => row.map((cell) => ({ ...cell })));

    // 先收集所有选中格子的颜色，然后决定每个的去向
    const displaced: { fromR: number; fromC: number; toR: number; toC: number; color: string; key: string }[] = [];
    ...
    for (const key of selectionMask) {
      ...
      const progress = (proj - projMin) / projRange;

      // 偏移距离：progress 越大越远 + 种子随机
      const baseDist = progress * maxDist;
      const dist = Math.round(baseDist + rng() * maxDist * 0.5);
      if (dist <= 0) continue; // 迎风面最前的不动

      // 方向加随机偏转
      const spread = (rng() - 0.5) * 0.6; // ±17°
      const a = Math.atan2(dirRow, dirCol) + spread;
      const dr = Math.round(Math.sin(a) * dist);
      const dc = Math.round(Math.cos(a) * dist);
      const toR = r + dr;
      const toC = c + dc;

      // 越界不处理（飘出画布消失）
      if (toR < 0 || toR >= M || toC < 0 || toC >= N) continue;

      displaced.push({ fromR: r, fromC: c, toR, toC, color: cell.color, key: cell.key });
    }

    // 先清空原始位置，再写入目标位置（避免自己覆盖自己）
    for (const { fromR, fromC } of displaced) {
      newData[fromR][fromC] = { ...transparentColorData };
    }
    for (const { toR, toC, color, key } of displaced) {
      // 目标位置是外部或空白则写入，非外部则跳过（不覆盖已有像素）
      const target = newData[toR][toC];
      if (target.isExternal || target.key === TRANSPARENT_KEY) {
        newData[toR][toC] = { key, color, isExternal: false };
      }
    }
    ...
    let newTotal = 0;
    newData.flat().forEach(c => {
      if (c && !c.isExternal && c.key !== TRANSPARENT_KEY) {
  ...
  // 键盘/滚轮监听：Esc 退出笔刷、Ctrl+滚轮 调整笔刷大小
  useEffect(() => {
    const keydown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && isBrushMode) {
        setIsBrushMode(false);
      }
    };
    const wheel = (e: WheelEvent) => {
      if (isBrushMode && (e.ctrlKey || e.metaKey)) {
        e.preventDefault();
        setBrushRadius(prev => {
          const delta = e.deltaY < 0 ? 2 : (e.deltaY > 0 ? -2 : 0);
          const next = prev + delta;
          return Math.max(1, Math.min(15, next));
        });
      }
    };
  ...
  // 差值补色自动恢复：页面加载时检测上次自动保存的数据
  useEffect(() => {
    if (originalImageSrc || mappedPixelData) return; // 已有数据则跳过
    try {
      const saved = localStorage.getItem('perlerBeads_autosave_pixelData');
      if (!saved) return;
      const parsed = JSON.parse(saved) as MappedPixel[][];
      const savedCounts = localStorage.getItem('perlerBeads_autosave_colorCounts');
      const savedDims = localStorage.getItem('perlerBeads_autosave_gridDimensions');
      const savedSys = localStorage.getItem('perlerBeads_autosave_colorSystem');
      if (!parsed || !savedCounts || !savedDims) return;
      setMappedPixelData(parsed);
      setColorCounts(JSON.parse(savedCounts));
      setGridDimensions(JSON.parse(savedDims));
      if (savedSys) setSelectedColorSystem(savedSys as ColorSystem);
      // 重新统计总量
      let total = 0;
      for (const row of parsed) {
        for (const cell of row) {
          if (cell && !cell.isExternal) total++;
        }
      }
  ...
  const handleAutoDetectNoise = () => {
    ...
    const noiseKeys = new Set<string>();
    for (const [hex, data] of Object.entries(colorCounts)) {
      if (data.count <= 3 && !excludedColorKeys.has(hex)) {
        noiseKeys.add(hex);
      }
    }
    setSuspectedNoiseKeys(noiseKeys);
    setHighlightNoiseHexes(noiseKeys.size > 0 ? new Set(noiseKeys) : null);
    setHighlightColorKey(null);
    if (noiseKeys.size === 0) {
      alert('未检测到疑似噪点（数量 ≤3 的未排除颜色）。');
    }
  };

  // 差值补色处理函数（支持批量：传入多个 hex 一次性处理）
  const handleInterpolateNoise = (hexKeys: string | Set<string>) => {
    const keys = typeof hexKeys === 'string' ? [hexKeys] : Array.from(hexKeys);
    if (keys.length === 0) return;
    if (!mappedPixelData || !gridDimensions || !colorCounts || !activeBeadPalette) return;
    ...
    // 收集所有待替换噪点坐标
    const noiseCoords: { row: number; col: number }[] = [];
    for (let j = 0; j < M; j++) {
      for (let i = 0; i < N; i++) {
        const cell = newMappedData[j][i];
        if (cell && !cell.isExternal && keySet.has(cell.color.toUpperCase())) {
          noiseCoords.push({ row: j, col: i });
        }
      }
    }

    if (noiseCoords.length === 0) return;

    // 对每个噪点，取 5×5 邻域（去除自身）找占比最高的色值
    for (const { row, col } of noiseCoords) {
      ...
      for (let dr = -2; dr <= 2; dr++) {
        for (let dc = -2; dc <= 2; dc++) {
          ...
          if (nr >= 0 && nr < M && nc >= 0 && nc < N) {
            const neighbor = newMappedData[nr][nc];
            if (neighbor && !neighbor.isExternal) {
              const nh = neighbor.color.toUpperCase();
              neighborCounts.set(nh, (neighborCounts.get(nh) || 0) + 1);
            }
          }
        }
      }
      ...
      let bestHex = '';
      let bestCount = 0;
      for (const [h, cnt] of neighborCounts) {
        if (cnt > bestCount) {
          bestCount = cnt;
          bestHex = h;
        }
      ...
      const cellHex = newMappedData[row][col].color.toUpperCase();
      if (bestHex && bestHex !== cellHex) {
        newMappedData[row][col].color = bestHex;
        newMappedData[row][col].key = bestHex;
      }
    }
    ...
    for (let j = 0; j < M; j++) {
      for (let i = 0; i < N; i++) {
        ...
        if (!newColorCounts[h]) {
          newColorCounts[h] = { count: 0, color: h };
        }
        newColorCounts[h].count++;
      }
    }
    ...
    // 从疑似噪点列表中移除所有已处理的颜色
    const nextNoise = new Set(suspectedNoiseKeys);
    for (const k of keys) nextNoise.delete(k);
    setSuspectedNoiseKeys(nextNoise);
    setHighlightNoiseHexes(nextNoise.size > 0 ? new Set(nextNoise) : null);

    // 清理已不在网格中的排除色
    const cleanExcluded = new Set(excludedColorKeys);
    for (const ek of excludedColorKeys) {
      if (!newColorCounts[ek.toUpperCase()]) cleanExcluded.delete(ek);
    }
    if (cleanExcluded.size !== excludedColorKeys.size) {
      setExcludedColorKeys(cleanExcluded);
    }
    ...
    // 自动保存到 localStorage
    try {
      localStorage.setItem('perlerBeads_autosave_pixelData', JSON.stringify(newMappedData));
  ...
  // 智能抠图：打开弹窗
  const handleBgRemoveOpen = () => {
    if (!originalImageSrc) {
      alert('请先上传图片');
      return;
    }
    setIsBgRemoveOpen(true);
  };

  // 智能抠图：应用结果
  const handleBgRemoveApply = (resultSrc: string) => {
    setOriginalImageSrc(resultSrc);
    setMappedPixelData(null);
    setGridDimensions(null);
    setColorCounts(null);
    setTotalBeadCount(0);
  ...
  const pixelateImage = (imageSrc: string, detailLevel: number, threshold: number, currentPalette: PaletteColor[], currentColorLimit?: number) => {
    ...
    img.onload = () => {
      ...
      console.log("Initial data mapping complete. Starting global color merging...");
  ...
  useEffect(() => {
    ...
    const handleToggleExcludeColor = (hexKey: string) => {
        ...
        if (isExcluding) {
            ...
            preExclusionCellsRef.current.set(hexKey, coords);
            ...
            preExclusionCellsRef.current.delete(hexKey);
    ...
    // 通过色号（如 "F06"）在当前色号系统中查找 hex 值
    const findHexByDisplayKey = (displayKey: string): string | null => {
      const upperKey = displayKey.toUpperCase().trim();
      if (!upperKey) return null;
      for (const color of fullBeadPalette) {
        if (getColorKeyByHex(color.hex, selectedColorSystem).toUpperCase() === upperKey) {
    ...
    // 替换被排除的颜色：将所有 excludedHex 的格子替换为 targetHex
    const handleReplaceColor = (excludedHex: string, targetHex: string) => {
      if (!mappedPixelData || !gridDimensions) return;

      const targetColor = fullBeadPalette.find(p => p.hex.toUpperCase() === targetHex);
      if (!targetColor) {
        alert(`未找到色号对应的颜色数据。`);
        return;
      }

      // 使用排除前保存的格子坐标，精确找到需要替换的位置
      const coords = preExclusionCellsRef.current.get(excludedHex);
      if (!coords || coords.length === 0) {
        alert('无法找到该颜色排除前的格子数据，请先恢复再重新排除。');
        return;
      }
      ...
      nextExcludedKeys.delete(excludedHex);
      preExclusionCellsRef.current.delete(excludedHex);
      setExcludedColorKeys(nextExcludedKeys);
      setMappedPixelData(newMappedData);
      ...
      // 重置替换输入状态
      setReplacingHexKey(null);
      setReplaceInputValue('');

      console.log(`Replaced ${replacedCount} cells from ${excludedHex} to ${targetHex}`);
    };
  ...
  return (
    ...
    <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center bg-gradient-to-b from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
            ...
            {!isManualColoringMode && (
              ...
              <div className="w-full md:max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                ...
                <div className="sm:col-span-2 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={handleBgRemoveOpen}
                    disabled={!originalImageSrc}
                    className="inline-flex items-center justify-center h-9 px-3 text-sm rounded-md border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-200 hover:bg-emerald-100 dark:hover:bg-emerald-800/40 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                  >
            ...
            <div className="w-full md:max-w-2xl">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                ...
                <div
                  ...
                  className="relative flex justify-center mb-3 sm:mb-4 bg-gray-100 dark:bg-gray-700 p-2 rounded-lg overflow-x-auto overflow-y-hidden"
                  ...
                  <PixelatedPreviewCanvas
                    ...
                    highlightNoiseHexes={highlightNoiseHexes}
                  ...
                  />
                  {/* 智能识别噪点按钮 */}
                  {!isManualColoringMode && mappedPixelData && colorCounts && (
                    <button
                      onClick={handleAutoDetectNoise}
                      title="智能识别噪点（数量≤3）"
                      className="absolute bottom-[10rem] right-4 z-10 w-10 h-10 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 bg-orange-500 text-white hover:bg-orange-600"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      {suspectedNoiseKeys.size > 0 && (
                        <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-bold">
                          {suspectedNoiseKeys.size}
                        </span>
                      )}
                    </button>
                  )}
                  ...
                  {/* 改色模式按钮 */}
                  <button
                    onClick={handleRecolorModeToggle}
                    disabled={!mappedPixelData}
                    title={isRecolorMode ? '退出改色模式' : '进入改色模式'}
                    className={`absolute bottom-[7rem] right-4 z-10 w-10 h-10 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ${
                      isRecolorMode
                  ...
                  <button
                    ...
                    disabled={!mappedPixelData}
                    title="一键加外边框 (H07)"
                    className="absolute bottom-16 right-4 z-10 w-10 h-10 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <button
                    ...
                    title={showGrid ? '关闭网格线预览' : '显示网格线'}
                    className={`absolute bottom-4 right-4 z-10 w-10 h-10 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ${
                      showGrid
                        ? 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-300 dark:border-gray-600'
                        : 'bg-purple-600 text-white hover:bg-purple-700'
                    }`}
                  ...
                  {/* 改色模式 — 紫色选中覆盖 + 弹窗 */}
                  {isRecolorMode && recolorSelected && mappedPixelData && gridDimensions && pixelatedCanvasRef.current && canvasContainerRef.current && (() => {
                    const canvas = pixelatedCanvasRef.current;
                    ...
                    const cellW = canvasRect.width / gridDimensions.N;
                    const cellH = canvasRect.height / gridDimensions.M;
                    const { row, col } = recolorSelected;
                    const cell = mappedPixelData[row]?.[col];
                    if (!cell || cell.isExternal) return null;

                    // Overlay position relative to parent container
                    ...
                    const neighbors: Array<{key: string; hex: string; direction: string}> = [];
                    for (const d of dirs) {
                      const nr = row + d.dr;
                      const nc = col + d.dc;
                      if (nr >= 0 && nr < gridDimensions.M && nc >= 0 && nc < gridDimensions.N) {
                        const n = mappedPixelData[nr][nc];
                        if (n && !n.isExternal && n.key !== TRANSPARENT_KEY) {
                          neighbors.push({
                            key: getColorKeyByHex(n.color.toUpperCase(), selectedColorSystem),
                            hex: n.color,
                            direction: d.label,
                          });
                        }
                    ...
                    return (
                      <>
                        {/* Purple overlay */}
                        <div
                          className="absolute z-10 pointer-events-none rounded-sm"
                          style={{
                            left: `${left}px`,
                            top: `${top}px`,
                            width: `${cellW * 0.9}px`,
        ...
        {!isManualColoringMode && originalImageSrc && colorCounts && Object.keys(colorCounts).length > 0 && (
          ...
          <div className="w-full md:max-w-2xl mt-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-100 dark:border-gray-700 color-stats-panel">
            ...
            <p className="text-xs text-center text-gray-500 dark:text-gray-400 mb-3">
              点击下方颜色可将其排除（非噪点）。总计: {totalBeadCount} 颗
              {suspectedNoiseKeys.size > 0 && (
                <span className="text-orange-500 font-medium"> | 疑似噪点: {suspectedNoiseKeys.size} 个颜色</span>
              )}
            </p>
            {suspectedNoiseKeys.size > 0 && (
              <div className="mb-3 p-2 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-semibold text-orange-600 dark:text-orange-400">
                    ⚡ 疑似噪点（数量≤3）— 点击色块高亮定位 · 点击差值补色智能替换
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      const totalBeads = Array.from(suspectedNoiseKeys).reduce((sum, h) => sum + (colorCounts[h]?.count || 0), 0);
                      if (!confirm(`确定批量差值补色全部 ${suspectedNoiseKeys.size} 种疑似噪点（共 ${totalBeads} 颗）？\n将用每个噪点周围 5×5 邻域占比最高的颜色替换。`)) return;
                      handleInterpolateNoise(new Set(suspectedNoiseKeys));
                    }}
                    className="text-xs py-1 px-3 bg-green-500 text-white rounded hover:bg-green-600 font-medium transition-colors shadow-sm whitespace-nowrap"
                  >
                    一键全部补色
                  </button>
                </div>
                <ul className="space-y-1 max-h-40 overflow-y-auto pr-1 text-sm">
                  {Array.from(suspectedNoiseKeys).sort(sortColorKeys).map((hexKey) => {
                    const displayColorKey = getColorKeyByHex(hexKey, selectedColorSystem);
                    const count = colorCounts[hexKey]?.count || 0;
                    const colorHex = colorCounts[hexKey]?.color || hexKey;
                    const isHighlighted = highlightNoiseHexes && highlightNoiseHexes.size === 1 && highlightNoiseHexes.has(hexKey);
                    ...
                    return (
                      <li
                        ...
                        className={`flex items-center justify-between p-1.5 rounded transition-colors ${
                          isHighlighted
                            ? 'bg-yellow-200 dark:bg-yellow-800/60 ring-2 ring-yellow-400'
                            : 'bg-orange-100 dark:bg-orange-800/30 hover:bg-orange-200 dark:hover:bg-orange-700/40'
                        }`}
                      >
                        <div className="flex items-center space-x-2">
                          {/* 点击色块 → 切换高亮：再点一次恢复显示全部噪点 */}
                          <span
                            className="inline-block w-4 h-4 rounded border border-orange-400 dark:border-orange-500 flex-shrink-0 cursor-pointer hover:scale-125 transition-transform"
                            style={{ backgroundColor: colorHex }}
                            onClick={(e) => {
                              e.stopPropagation();
                              if (isHighlighted) {
                                // 当前只高亮这一种 → 恢复显示全部
                                setHighlightNoiseHexes(new Set(suspectedNoiseKeys));
                              } else {
                                // 只高亮这种颜色
                                setHighlightNoiseHexes(new Set([hexKey]));
                              }
                              setHighlightColorKey(null);
                            }}
                            title="点击高亮该颜色（再点恢复显示全部噪点）"
                          />
                          <span className="font-mono font-medium text-orange-800 dark:text-orange-200 text-xs">{displayColorKey}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="text-xs text-orange-600 dark:text-orange-400 font-medium">{count} 颗</span>
                          {/* 差值补色按钮 */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (confirm(`确定对色号 ${displayColorKey} 执行差值补色？\n将用周围 5×5 邻域占比最高的颜色替换全部 ${count} 个噪点。`)) {
                                handleInterpolateNoise(hexKey);
                              }
                            }}
                            className="text-xs py-0.5 px-2 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 rounded hover:bg-green-200 dark:hover:bg-green-800/40 font-medium"
                            title="取周围24格占比最高颜色替换"
                          >
                            差值补色
                          </button>
                          {/* 取消标记 */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              const next = new Set(suspectedNoiseKeys);
                              next.delete(hexKey);
                              setSuspectedNoiseKeys(next);
                              setHighlightNoiseHexes(next.size > 0 ? new Set(next) : null);
                            }}
                            className="text-xs py-0.5 px-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"
                            title="取消噪点标记"
                          >
                            ✕
                          </button>
                ...
                <button
                  onClick={() => { setSuspectedNoiseKeys(new Set()); setHighlightNoiseHexes(null); }}
            ...
            <ul className="space-y-1 max-h-60 overflow-y-auto pr-2 text-sm">
              {Object.keys(colorCounts)
                .filter(hex => !suspectedNoiseKeys.has(hex))
                .sort(sortColorKeys)
                .map((hexKey) => {
                    ...
                    <div className="mt-2 border border-gray-200 dark:border-gray-700 rounded-md p-2 bg-gray-100 dark:bg-gray-800">
                        ...
                        {Array.from(excludedColorKeys).length > 0 ? (
                          <ul className="space-y-1">
                            {Array.from(excludedColorKeys).sort(sortColorKeys).map(hexKey => {
                              ...
                              return (
                                <li key={hexKey} className="flex flex-col p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded">
                                  <div className="flex justify-between items-center w-full">
                                    ...
                                    <div className="flex items-center gap-1">
                                      {replacingHexKey === hexKey ? (
                                        <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                                          <span className="text-xs text-gray-500 dark:text-gray-400">替换为</span>
                                          <input
                                            type="text"
                                            ...
                                            onKeyDown={e => {
                                              if (e.key === 'Enter') {
                                                const targetHex = findHexByDisplayKey(replaceInputValue);
                                                if (targetHex) {
                                                  handleReplaceColor(hexKey, targetHex);
                                                } else {
                                                  alert(`未找到色号 "${replaceInputValue}"，请检查输入。`);
                                                }
                                            ...
                                            placeholder="如 F06"
                                            className="w-16 text-xs px-1.5 py-0.5 border border-gray-300 dark:border-gray-500 rounded bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-purple-500"
                                            autoFocus
                                          />
                                          <button
                                            onClick={() => {
                                            ...
                                            className="text-xs py-0.5 px-1.5 bg-purple-500 text-white rounded hover:bg-purple-600"
                                          >
                                            确定
                                          </button>
                                        </div>
                                        ...
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setReplacingHexKey(hexKey);
                                            setReplaceInputValue('');
                                          }}
                                          className="text-xs py-0.5 px-2 bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 rounded hover:bg-purple-200 dark:hover:bg-purple-800/40"
                                        >
                                          替换为
                                        </button>
                                      )}
                      ...
                      <button
                        onClick={() => {
                          ...
                          preExclusionCellsRef.current.clear();
                          setRemapTrigger(prev => prev + 1);
                          setIsManualColoringMode(false);
                          setSelectedColor(null);
      ...
      <FloatingToolbar
        ...
        isBrushMode={isBrushMode}
        onToggleBrush={handleBrushToggle}
        brushRadius={brushRadius}
        onBrushRadiusChange={setBrushRadius}
        undoCount={undoCount}
        onUndo={handleUndo}
        isDissolvePanelOpen={isDissolvePanelOpen}
        onToggleDissolvePanel={() => {
          const next = !isDissolvePanelOpen;
          setIsDissolvePanelOpen(next);
          setDissolvePanelActive(next);
        }}
        selectionCount={selectionMask.size}
      />
      ...
      {isManualColoringMode && (
        <DissolvePanel
          isOpen={isDissolvePanelOpen}
          onToggleOpen={() => {
            setIsDissolvePanelOpen(!isDissolvePanelOpen);
            setDissolvePanelActive(!isDissolvePanelOpen);
          }}
          onApply={handleDissolveApply}
          selectionCount={selectionMask.size}
          isActive={dissolvePanelActive}
          onActivate={() => setDissolvePanelActive(true)}
        />
      )}
